...
const Paper = ({ paperDetails, index }) => {
  ...
  function expand(index, section) {
    ...
    const buttonText = document.getElementsByClassName("expand-" + section)[
      index
    ].innerHTML;

    if (buttonText === "Expand") {
      document.getElementsByClassName(className)[index].style.display = "block";
      document.getElementsByClassName("expand-" + section)[index].innerHTML =
        "Collapse";
  ...
  return (
    <div>
      <Link to={`/paper/${paperDetails.paperId}/${paperDetails.vertical_id}`}>
      ...
      <div className="authors">
        {paperDetails.authors.map((author) => (
          <>
            <Link to={`/author/${author.authorId}/${paperDetails.vertical_id}`}>
      ...
      <>
        {paperDetails.s2FieldsOfStudy.map(
          (field) =>
            field.source === "s2-fos-model" && (
              <span className="detail">{field.category}</span>
            )
        )}
      ...
      {paperDetails.isOpenAccess && (
        <div className="pdf">
          ...
          <a
            href={paperDetails.openAccessPdf && paperDetails.openAccessPdf.url}
            target="_blank"
          >
            PDF
          </a>
      ...
      <>
        {paperDetails.citations && (
          <>
            ...
            <span>
              ({paperDetails.influentialCitationCount}&nbsp;Influential
              Citations)&nbsp;
            </span>
            <span
              className="expand-citations"
              onClick={() => expand(index, "citations")}
            >
              Expand
            </span>
            <div className="citations" style={{ display: "none" }}>
              ...
              {paperDetails.citations.map((citation) =>
                paperDetails.depth == 0 ? (
                  <Link
                    to={`/paper/${citation.paperId}/${paperDetails.vertical_id}`}
                  >
                    &nbsp;&nbsp;•&nbsp;&nbsp;{citation.title}
                  </Link>
                ) : (
      ...
      <>
        {paperDetails.references && (
          <>
            ...
            <span
              className="expand-references"
              onClick={() => expand(index, "references")}
            >
              Expand
            </span>
            <div className="references" style={{ display: "none" }}>
              ...
              {paperDetails.references.map((reference) =>
                paperDetails.depth == 0 ? (
                  <Link
                    to={`/paper/${reference.paperId}/${paperDetails.vertical_id}`}
                  >
                    &nbsp;&nbsp;•&nbsp;&nbsp;{reference.title}
                  </Link>
                ) : (
