...
const TaskDetails = () => {
  ...
  useEffect(() => {
    ...
    const getTaskDetails = async () => {
      ...
      const _patentDetails = await fetchPatentDetails();
      console.log("patent", _patentDetails);
      setPatentDetails(_patentDetails);
  ...
  const fetchTaskDetails = async () => {
    const url =
      process.env.REACT_APP_FLASK_WEBSERVER + "vertical_details?id=" + id;
  ...
  const fetchFundingDetails = async () => {
    const url =
      process.env.REACT_APP_FLASK_WEBSERVER + "funding_details?id=" + id;
    const res = await fetch(url);
    const data = await res.json();
  ...
  useEffect(() => {
    ...
    const updateTask = async () => {
      const res = await fetch(
        process.env.REACT_APP_FLASK_WEBSERVER + "update_vertical",
        {
          method: "PATCH",
          headers: {
  ...
  async function saveEdits(subHeading) {
    ...
    setTaskDetails({
      ...taskDetails,
      [subHeading]: document.getElementById(subHeading).innerHTML,
    });
    const res = await fetch(
      process.env.REACT_APP_FLASK_WEBSERVER + "task_details/" + id,
      {
        method: "PUT",
        headers: {
          "Content-type": "application/json",
    ...
    <div style={{ width: "75%" }}>
        ...
        {activeTab == "funding" ? (
          Object.keys(fundingDetails).length > 0 ? (
            <div>
              ...
              {Object.entries(fundingDetails).map(
                ([search_term, grants]) =>
      ...
      {task.status === "Rejected" && (
        <div>
          <FontAwesomeIcon
            icon={faCircleXmark}
            style={{ height: "75px", color: "#e14141", paddingTop: "30px" }}
          />
          <br />
      ...
      {task.status === "Approved" && (
        <div>
          <FontAwesomeIcon
            icon={faCircleCheck}
            style={{ height: "75px", color: "#10a37f", paddingTop: "30px" }}
          />
          <br />
